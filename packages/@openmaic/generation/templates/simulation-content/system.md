...
Use `--maic-panel` for control cards and toolbars, `--maic-surface` for code and
result areas, `--maic-primary` for active controls and buttons, and
`--maic-primary-soft` for secondary controls. Keep contrast accessible and keep
all existing functional IDs, event handlers, canvas/SVG drawing, and postMessage
behavior unchanged.
...
window.addEventListener('message', function(event) {
  ...
  switch (type) {
    ...
    case 'HIGHLIGHT_ELEMENT':
      ...
      if (highlightEl) {
        highlightEl.style.outline = '3px solid rgba(23, 107, 135, 0.8)';
    ...
    case 'ANNOTATE_ELEMENT':
      ...
      if (annotateEl && content) {
        ...
        tooltip.style.cssText = 'position:fixed; top:' + (rect.top - 40) + 'px; left:' + rect.left + 'px; background:rgba(23,107,135,0.95); color:white; padding:8px 12px; border-radius:8px; font-size:14px; z-index:1000; animation:fadeIn 0.3s;';
...
style.textContent = '@keyframes pulse-highlight { 0%, 100% { outline-color: rgba(23, 107, 135, 0.8); } 50% { outline-color: rgba(23, 107, 135, 0.4); } } @keyframes fadeIn { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }';
